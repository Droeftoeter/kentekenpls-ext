...
test("Category selector closes after pressing close button", async ({
  page,
}) => {
...
test("Clicking on a folder moves the navigator into the folder", async ({
  page,
}) => {
  const popout = page.getByTestId("kentekenpls-window");
...
test("Clicking on the arrow returns user to the parent folder", async ({
  page,
}) => {
  const popout = page.getByTestId("kentekenpls-window");
...
test("Picking a category injects a license plate into the focused field", async ({
  page,
}) => {
  const popout = page.getByTestId("kentekenpls-window");
