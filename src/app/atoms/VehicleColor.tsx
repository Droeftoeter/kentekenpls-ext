...
import { RdwOpenDataVehicleColor } from "../../background/api";
...
const VehicleColor = ({
  ...
  const colorNames = [primaryColor, secondaryColor].filter(
    (c) => c && c !== "Niet geregistreerd",
  ) as RdwOpenDataVehicleColor[];
  ...
  return colorNames.length ? (
    ...
    >
      ...
      <rect width="16" height="16" fill={ColorMap[colorNames[0]]} />
      {colorNames.length > 1 && (
        <polygon points="16,0 16,16 0,16" fill={ColorMap[colorNames[1]]} />
      )}
    </svg>
  ) : null;
};
